...
(defun branch1 (queue finish waypoint)            ;helper fcn for branch
  (cond 
    ...
    ((and (equal finish (caar queue)) (member waypoint (car queue))) 
      (print queue)
      (reverse (car queue)))
    (t ;(print queue)
      (branch1 
        (sort                ;new predicate used
          (append
...
(defun expand (path)
  (remove-if #'
    (lambda (path) (member (car path) (cdr path))) ;kill cycles
    (mapcar #'(lambda (child) (cons child path))
      ; Get the children of each room from the ROOMS map
